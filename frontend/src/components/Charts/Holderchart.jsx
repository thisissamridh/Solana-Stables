...
const HolderChart = ({ coinName }) => {
    ...
    useEffect(() => {
        ...
        if (
            !holderData ||
            !holderData[referenceCoin] ||
            !holderData[referenceCoin].TotalHolders
        )
            return;
        const referenceDates = holderData[referenceCoin].TotalHolders.map(
            (entry) => new Date(entry.datetime * 1000).toISOString().split('T')[0]
        );
        ...
        const chartDataArray = Object.values(data).sort(
    ...
    return (
        <div className="bg-black-gradient p-4 flex flex-col shadow-xl rounded-lg relative w-full h-96">
            <div className="flex items-center  justify-between">
                <strong className="text-white-700 bg-black-gradientfont-bond text-gradient px-2 py-3"> Stablecoin Holder Trend</strong>
            ...
            <div className="w-full h-[22rem] bg-black-gradient p-4 rounded-md flex flex-col flex-1 shadow-xl">
                ...
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        <Legend />
                        {selectedOption === 'totalHolders' || selectedOption === 'both' ? (
                            <Line
                                type="monotone"
                        ...
                        {selectedOption === 'activeHolders' || selectedOption === 'both' ? (
                            <Line type="monotone" dataKey="activeHolders" stroke="#82ca9d" />
                        ) : null}
                        <Brush />
                    </LineChart>
                </ResponsiveContainer>
